...
PATH_TO_YCMD = os.path.join( DIR_OF_THIS_SCRIPT, '..', 'build', 'Debug', 'src', 'ycmd' )
...
class YcmdHandle( object ):
  ...
  @classmethod
  def ConnectToServer( cls, port ):
    hmac_secret = os.urandom( HMAC_SECRET_LENGTH )
    return cls( None, port, hmac_secret )

  def IsAlive( self ):
    if self._popen_handle is None:
      return True
...
def Main():
  ...
  parser.add_argument( '--connect', type = int,
                       ...
                       metavar = 'port',
                       dest = 'port',
                       help = 'Connect to an existing ycmd server' )
  ...
  if not args.port:
    ...
    server = YcmdHandle.ConnectToServer( args.port )
